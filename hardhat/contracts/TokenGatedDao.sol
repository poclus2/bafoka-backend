...
contract TokenGatedDao {
    ...
    function endVoting(uint256 _proposalIndex) external { 
        require(_proposalIndex < proposalCount, "Invalid proposal index");
        ...
        if (proposal.yesVotes > proposal.noVotes) {
            ...
            emit ProposalExecuted(_proposalIndex, true);
        } else {
            // Reject the proposal if it has more no votes
            emit ProposalExecuted(_proposalIndex, false);
        }
    }
    
    event ProposalExecuted(uint256 indexed proposalIndex, bool executed);
